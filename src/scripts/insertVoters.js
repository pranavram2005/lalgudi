...
// ── CONFIG ─────────────────────────────────────────────────────────────────
const VOTERS_FOLDER = resolve('./src/voters')   // 👈 change this path if needed
const REPEAT        = 1
const BATCH         = 100
const DELAY_MS      = 50
// ───────────────────────────────────────────────────────────────────────────
...
// ── Load all .jsx files from the folder ────────────────────────────────────
console.log(`📂 Scanning folder: ${VOTERS_FOLDER}\n`)
...
if (jsxFiles.length === 0) {
  console.error('❌ No .jsx/.js files found in the voters folder. Exiting.')
  process.exit(1)
}

console.log(`📄 Found ${jsxFiles.length} file(s):`)
...
for (const filePath of jsxFiles) {
  ...
  if (Array.isArray(data)) {
    ...
    console.warn(`   ⚠️  Skipped ${filePath} — default export is not an array`)
  }
}

// ── Build the full dataset ──────────────────────────────────────────────────
const allData = Array.from({ length: REPEAT }, () => allRawData).flat().map(mapVoter)

console.log(`\nTotal voters to upsert (${REPEAT}x repeated): ${allData.length}`)
